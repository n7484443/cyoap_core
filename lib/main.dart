...
int _getSizeInternal(List<dynamic> pos) {
  Pos innerPos = listToPos(pos);
  return platform.getChoiceNode(innerPos)!.getMaxSize(platform, true);
}
...
String _getSizeDataListInternal(List<dynamic> pos, String alignment, int maxChildrenPerRow) {
  ...
  (List<List<SizeData>>, int)? out = choice?.getSizeDataList(align: align, maxChildrenPerRow: maxChildrenPerRow, showAll: false, platform: platform);
  if (out == null) {
    return jsonEncode([]);
  }
